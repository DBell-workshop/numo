# ClarSum Privacy Policy

> Public copy lives at <https://dbell-workshop.github.io/numo/PRIVACY.html>. This URL is the one filed with App Store Connect. (The `numo` slug in the URL is a legacy artifact from the working name; product identity is **ClarSum**.)

---

**Effective Date**: 2026-06-04
**App**: ClarSum: AI Calculator
**Developer**: Beijing Linkos Culture & Technology Co., Ltd. (北京灵可司文化科技有限公司)
**Contact**: support@linkos.cc

## TL;DR

- **ClarSum Pro** (one-time purchase) collects **zero data**. Math, voice recognition, and receipt OCR all run on your device. No ads, no network, no tracking.
- **ClarSum Free** runs the same on-device math, voice, and OCR. But it also shows **one launch-screen ad** per cold start, served by Google AdMob. The ad request may collect device identifiers (subject to your iOS App Tracking Transparency choice). Everything else is still on-device.
- You can opt out of personalised ads by declining the ATT prompt on first launch, and you can remove ads entirely by upgrading to ClarSum Pro.

## What ClarSum does on your device (and only on your device)
...
- **Calculator state and chat timeline** — your last expression, hero result, and the chat-style history of past calculations. Persisted to your local `UserDefaults` so the app resumes where you left off. Never read by anyone but ClarSum, on this device.
...
ClarSum Free shows **one App Open ad** per cold launch via **Google AdMob** (Google's mobile advertising SDK, version 13.x). To serve and measure that ad, AdMob and its supporting framework (User Messaging Platform, UMP) may collect:
...
### ClarSum Pro disables all of the above

Purchasing **ClarSum Pro** (a one-time, non-consumable in-app purchase) removes the ad-serving code path entirely. After upgrading:
...
If you have already accepted the ATT prompt before upgrading, you can revoke it any time via **iOS Settings → Privacy & Security → Tracking → ClarSum**.

## Permissions ClarSum asks for
...
ClarSum does **not** request access to your contacts, photos library (the camera permission scans only the live viewfinder, never your stored photos), calendar, location services, health data, motion, or HomeKit.

You can revoke any permission at any time via iOS **Settings → Privacy & Security → [Permission name] → ClarSum**.

## Network usage

- **ClarSum Pro**: zero network requests. The app functions identically in airplane mode.
- **ClarSum Free**: one ad request per cold launch, sent to Google AdMob's endpoint. No other ClarSum feature uses the network. Voice recognition, OCR, math evaluation, and the chat timeline are all 100% local. The app continues to work in airplane mode, only without the launch ad.

## Third-party services

ClarSum Free embeds the following third-party SDKs:
...
**ClarSum Pro** strips all of the above out of the runtime — even though the code paths exist in the binary, they are gated by a Pro check at startup and never execute.

## Mainland-China users (PIPL compliance)

On first launch in the mainland-China region, ClarSum presents a **PIPL transparency sheet** explaining the above data flows, with separate consents for:
...
ClarSum is rated 4+ on the App Store. We do not knowingly collect data from children. ClarSum Free's ad SDK serves only non-personalised, family-safe ads to users under 13 (per AdMob's standard child-directed treatment), and ClarSum Pro shows no ads at all. The other data flows (OCR, voice, math) are 100% on-device for all ages.

## Changes to this policy

If we change ClarSum's privacy posture, we will update this document **and** the App Store Privacy Nutrition Label before the new version ships. Substantive changes are reflected in the **Effective Date** above.
...
To clear all local ClarSum state on your device:

1. **iOS Settings → General → iPhone Storage → ClarSum → Offload App** (preserves data) or **Delete App** (wipes everything).
...
*This document is the canonical privacy policy for ClarSum. The App Store Privacy Nutrition Label is filled in to match: ClarSum Pro = "Data Not Collected"; ClarSum Free = "Identifiers (Linked for Third-Party Advertising)" and "Diagnostics (Linked for Third-Party Advertising)" via the AdMob SDK, contingent on the user's ATT choice.*
